...
contract GangCrowdFund is ERC721URIStorage, ReentrancyGuard {
    ...
    constructor(GangCrowdFundOpts memory _GangCrowdFundOpts)
        ERC721(_GangCrowdFundOpts.name, _GangCrowdFundOpts.symbol)
    {
        owner = msg.sender;
    ...
    function projectOwnerAndGuarantorWithdrawRevenue() external payable onlyOwner nonReentrant {
        require(
            block.number > initialBlock + investPhase + investorsWithdrawPhase,
            "invest time is not over or withdraw time not yet come"
        );
        address payable projectOwner = payable(owner);
        address payable guarantor = payable(guarantor);
        uint256 projectOwnerTotalRevenue = ((totalRevenue - investorTotalRevenue) * projectOwnerRevenueShare)
            / (projectOwnerRevenueShare + guarantorRevenueShare);
        uint256 projectOwnerTotalRoyalty = ((totalRoyalty - investorTotalRoyalty) * projectOwnerRoyaltyShare)
            / (projectOwnerRoyaltyShare + guarantorRoyaltyShare);
        uint256 projectOwnerTotalShare = projectOwnerTotalRevenue + projectOwnerTotalRoyalty;
        uint256 projectOwnerAllowedWithdrawRevenue = projectOwnerTotalShare - alreadyWithdrawRevenue[owner];
        alreadyWithdrawRevenue[owner] += projectOwnerAllowedWithdrawRevenue;
        uint256 guarantorTotalRevenue = ((totalRevenue - investorTotalRevenue) * guarantorRevenueShare)
            / (projectOwnerRevenueShare + guarantorRevenueShare);
        uint256 guarantorTotalRoyalty = ((totalRoyalty - investorTotalRoyalty) * guarantorRoyaltyShare)
            / (projectOwnerRoyaltyShare + guarantorRoyaltyShare);
        uint256 guarantorTotalShare = guarantorTotalRevenue + guarantorTotalRoyalty;
        uint256 guarantorAllowedWithdrawRevenue = guarantorTotalShare - alreadyWithdrawRevenue[guarantor];
        alreadyWithdrawRevenue[guarantor] += guarantorAllowedWithdrawRevenue;
    ...
    function investorWithdrawInvest(uint256 amount) external payable nonReentrant returns (uint256) {
        ...
        require(
            block.number >= initialBlock + investPhase + investorsWithdrawPhase
                || block.number <= initialBlock + investPhase,
            "invest time is over or withdraw time not yet come"
        );
        address payable receiver = payable(msg.sender);
    ...
    function investorWithdrawRevenue() external payable nonReentrant {
        address payable receiver = payable(msg.sender);
        require(investment[msg.sender] > 0);
        require(
            block.number > initialBlock + investPhase
        ...
        if (
            totalRevenue < (totalInvestment * insuranceThreshold) / 1000 && insuranceCompensated == false
                && block.number > initialBlock + investPhase + ((investorsWithdrawPhase * 950) / 1000)
        ) {
    ...
    function investManagerWithdrawInvest(uint256 amount) external payable nonReentrant {
        require(investment[msg.sender] > 0);
        require(amount <= investment[msg.sender]);
        require(
            block.number >= initialBlock + investPhase + investorsWithdrawPhase
    ...
    function investManagerWithdrawRevenue() external payable nonReentrant returns (uint256 received) {
        address payable receiver = payable(msg.sender);
        require(investment[msg.sender] > 0);
        require(
            block.number > initialBlock + investPhase
        ...
        bytes memory data = abi.encodeWithSignature(
            "withdrawTargetRevenue(address,uint256)", address(this), investManagerAllowedWithdraw
        );
    ...
    // view functions
    function getCrowdfundPhase() external view returns (string memory) {
        if (block.number <= initialBlock + investPhase) {
            return "invest";
        } else if (block.number <= initialBlock + investPhase + investorsWithdrawPhase) {
            return "withdraw";
        } else if (block.number <= initialBlock + investPhase + investorsWithdrawPhase + investorsExitPhase) {
            return "exit";
        } else {
            return "end";
        }
